...
class FavoritePlantContainer extends StatelessWidget {
  const FavoritePlantContainer({
    super.key, required this.plant, required this.notifyParent,
  });
          ...
          Row(
            children: [
              Stack(
                ...
                children: [
                  SvgPicture.asset("assets/images/favorite_plant_bg.svg", colorFilter: ColorFilter.mode(Color(int.parse(plant.color)), BlendMode.srcIn),),
                  Positioned(
                    bottom: -5,
                    right: 0,
                    child: Image.asset(plant.imagePath, scale: 2.5,)),
                ],
              ),
            Padding(
            padding: const EdgeInsets.only(bottom: 24, left: 10),
            child: Text(plant.name, style: const TextStyle(fontFamily: "Philosopher", fontSize: 15, fontWeight: FontWeight.bold),),
          ),
            ],
          ),
          Padding(
